...
export default async function SettingsPage() {
  ...
  const [{ data: profile }, { data: certifications }, { data: workExperience }] = await Promise.all([
    ...
    supabase.from("work_experience").select("*").eq("profile_id", user.id).order("start_date", { ascending: false }),
  ]);
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-sm-bg">
        <SettingsForm profile={profile} workExperience={(workExperience ?? []) as WorkExperience[]} />
