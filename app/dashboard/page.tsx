...
export default async function DashboardPage() {
  ...
  const verifiedCount = typedProjects.filter(
  ...
  const completionItems = [
    ...
    { label: "Add work experience", done: (workExp?.length ?? 0) > 0 },
    { label: "Add a project", done: typedProjects.length > 0 },
    { label: "Get a project verified", done: verifiedCount > 0 },
  ];
    ...
    <main className="min-h-screen bg-sm-bg">
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <StatCard label="Profile Score" value={`${completionScore}%`} />
        </div>
          ...
          <div className="md:col-span-1 space-y-4">
            ...
            <div className="bg-white border border-border rounded-xl p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs font-bold text-accent">{completionScore}%</span>
              </div>
              <div className="w-full bg-sm-bg rounded-full h-1.5 mb-4">
                <div
              ...
              <ul className="space-y-2">
                {completionItems.map((item) => (
                  <li key={item.label} className="flex items-center gap-2.5 text-xs">
                    <span className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${item.done ? "bg-emerald-100" : "bg-sm-bg border border-border"}`}>
                      {item.done && (
                    ...
                    <span className={item.done ? "text-text-dim line-through" : "text-navy"}>
                      {item.label}
                    </span>
                  </li>
                ))}
